...
export function apply(ctx, config) {
  ...
  ctx.inject?.(['webServer', 'workspaceRegistry', 'sessions', 'agents', 'sessionPersistence', 'defaultCwd'], (webCtx) => {
    ...
    const disposer = webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          await booted
          if (p === '/status') { const b = bridgeView(); return send(res, { ok: true, plugin: 'wxbridge', version: '0.0.15', hostPid: process.pid, bridge: b, peers: b ? b.peers : 0 }) }
          ...
          const runTask = async (text, opts = {}) => {
            ...
            if (!sessionId) {
              ...
              try {
                await sessionsSvc.create(sessionId, { meta: { cwd } })
              } catch (e) {
                // 部分版本 create 只接受 id 字符串
                await sessionsSvc.create(sessionId)
              }
              created = true
            }
            ...
            if (!agent) return { ok: false, error: '无法取得会话 agent: ' + sessionId, sessionId, created }
