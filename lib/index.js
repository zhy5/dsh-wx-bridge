...
const ACTION_LOG_MAX = 60
  ...
  const loadSessionHelpers = async () => {
    if (createUserMessage) return createUserMessage
    for (const spec of ['@deepseek-ai/dsh-llm', '@deepseek-ai/dsh-session', 'dsh-llm']) {
      try {
        const m = await import(spec)
        if (typeof m.createUserMessage === 'function') { createUserMessage = m.createUserMessage; return createUserMessage }
        if (m.internals && typeof m.internals.createUserMessage === 'function') { createUserMessage = m.internals.createUserMessage; return createUserMessage }
      } catch {}
    }
    return null
  }
...
export function apply(ctx, config) {
  ...
  const boot = async () => {
    ...
    try {
      let port = 0
      try {
        const srv = webCtx.webServer && (webCtx.webServer.server || webCtx.webServer.httpServer || webCtx.webServer.instance)
  ...
  ctx.inject?.(['webServer', 'workspaceRegistry', 'sessions', 'agents', 'sessionPersistence'], (webCtx) => {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const runTask = async (text, opts = {}) => {
            const sessionsSvc = webCtx.sessions
            const agents = webCtx.agents
            if (!sessionsSvc || !agents) return { ok: false, error: 'sessions/agents 服务不可用' }
            ...
            const ws = (opts.workspaceId ? workspaces.find((w) => String(w.id ?? w.workspaceId) === String(opts.workspaceId)) : null)
              ...
              || workspaces.find((w) => norm(w.path) === norm(opts.cwd || '')) || workspaces[0]
            ...
            if (sessionId) {
              // 复用已有会话：确认它确实存在
              let exists = false
              try { exists = !!sessionsSvc.get(sessionId) } catch {}
              if (!exists) {
                try {
                  const recs = await webCtx.sessionPersistence.list()
                  exists = (recs || []).some((r) => String((r.header || r).id) === sessionId)
                } catch {}
              }
              if (!exists) sessionId = ''
            }
            if (!sessionId) { sessionId = 'session-' + randomUUID(); created = true }
            try { await ws.attachSession(sessionId) } catch (e) { /* 已登记或 cwd 不匹配时忽略 */ }

            // 取 agent：活会话 → resume（挂到已持久化的会话）→ create（新建，headless 同款）
            let agent = (() => { try { return agents.get(sessionId) } catch { return undefined } })()
            const tryErrs = []
            if (!agent && !created) {
              try { const r = await agents.resume({ sessionId }); agent = (r && r.agent) ? r.agent : r } catch (e) { tryErrs.push('resume: ' + String(e?.message ?? e)) }
            }
            if (!agent) {
              try { const r = await agents.create({ sessionId, meta: { cwd } }); agent = (r && r.agent) ? r.agent : r } catch (e) { tryErrs.push('create: ' + String(e?.message ?? e)) }
            }
            if (!agent || !agent.session) {
              return { ok: false, error: '未获得可用 agent（' + tryErrs.join(' | ') + '）', sessionId, created }
            }

            const mkMsg = await loadSessionHelpers()
            if (!mkMsg) return { ok: false, error: '无法加载 createUserMessage（@deepseek-ai/dsh-session 不可用）', sessionId }
            const firstSeq = agent.session.seq
            const msg = mkMsg({ content: [{ type: 'text', text: String(text) }], source: { kind: 'user' } })
            agent.followup(msg)
            await agent.whenIdle()
            try { await sessionsSvc.flush(agent.session) } catch {}

            // 照官方 summarize 的口径从事件流取回复
            const events = agent.session.events || []
            let started = false, out = '', reason = null
            for (const ev of events) {
              ...
              if (ev.type === 'turn/end') reason = ev.data && ev.data.reason
            }
            // 调试回执文件（临时）：供外部核对回复提取是否成功
            try {
              const dbg = join(keeper ? keeper.dataDir : KERNEL, 'task-debug.jsonl')
              appendFileSync(dbg, JSON.stringify({ at: new Date().toISOString(), sessionId, created, seqFrom: firstSeq, seqTo: agent.session.seq, eventTypes: events.filter((e) => e.seq >= firstSeq).slice(-6).map((e) => e.type), text: out.slice(0, 200), reason }) + NL)
            } catch {}

            pushAction({ action: 'task', ok: !!out, detail: 'session=' + sessionId + ' created=' + created + ' len=' + out.length })
            return { ok: true, sessionId, created, text: out, reason: reason ? String(reason.kind || reason) : null }
          }
          ...
          if (p === '/svc') {
            ...
            return send(res, { ok: true, services: seen, context: ctxSeen, workspaceRegistryFace: reg, sessionsFace: ses, agentsFace: ag })
          }
