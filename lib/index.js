  ...
  ctx.inject?.(['webServer', 'workspaceRegistry', 'sessions', 'agents', 'sessionPersistence', 'agentDefaultModel', 'sessionController'], (webCtx) => {
    ...
    const bridgeView = () => {
      ...
      return {
        ...
        allowlist: String((() => { try { return readConfigFile().allowlist || 'strict' } catch { return 'strict' } })()),
      }
    }
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (p === '/config' && req.method === 'POST') {
            ...
            const f = configPath()
            const keep = readJsonFile(f)
            for (const k of ['dataDir', 'cwd', 'vault', 'dshBin', 'intervalMs', 'staleMs', 'allowlist']) if (body && body[k]) keep[k] = body[k]
          ...
          if (p === '/push' && req.method === 'POST') {
            // 主动推送：往桥的 outbox 丢一条（桥每 5 秒代发）。桌面会话 / 面板 / 其它插件都能调。
            const body = await readBody(req)
            const text = String((body && (body.text || body.message)) || '').trim()
            if (!text) return send(res, { ok: false, error: 'text 不能为空' }, 400)
            const dir = keeper ? keeper.dataDir : ''
            if (!dir) return send(res, { ok: false, error: '桥的数据目录未知（桥没在运行？）' }, 503)
            try {
              const outbox = join(dir, 'outbox')
              mkdirSync(outbox, { recursive: true })
              const file = join(outbox, Date.now() + '-' + Math.random().toString(16).slice(2, 10) + '.json')
              ...
              pushAction({ action: 'push', ok: true, detail: text.slice(0, 40) })
              return send(res, { ok: true, file, note: '已排队；桥最多 5 秒内发出（结果看 outbox/sent 或 failed）' })
            } catch (e) { return send(res, { ok: false, error: String(e?.message ?? e) }, 500) }
          }
          if (p === '/approval') {
            const f = configPath()
            const keep = readJsonFile(f)
            const cur = String((keep.acp && keep.acp.permPolicy) || '').toLowerCase() || 'ask'
            if (req.method !== 'POST') return send(res, { ok: true, approval: cur, configFile: f, modes: ['ask', 'allow', 'reject'] })
            // 审批策略：ask（推到微信等回复）/ allow（自动批准）/ reject（一律拒绝）
            const body = await readBody(req)
            const mode = String((body && body.mode) || '').toLowerCase()
            if (!['ask', 'allow', 'reject'].includes(mode)) return send(res, { ok: false, error: 'mode must be ask | allow | reject' }, 400)
            keep.acp = Object.assign({}, (keep.acp && typeof keep.acp === 'object') ? keep.acp : {}, { permPolicy: mode })
            mkdirSync(dirname(f), { recursive: true })
            writeFileSync(f, JSON.stringify(keep, null, 2))
            pushAction({ action: 'approval', ok: true, detail: mode })
            let restarted = 0
            if (body && body.restart && keeper) { try { restarted = keeper.startBridge('approval-change') || 0 } catch (e) { ctx.logger?.warn?.('wxbridge: 重启失败 ' + String(e?.message ?? e)) } }
            return send(res, { ok: true, approval: mode, configFile: f, restarted,
              note: restarted ? '已写入配置并重启桥，立即生效' : '已写入配置；桥每条任务重读配置，手机发 /审批 也能立即改' })
          }
          if (p === '/allowlist' && req.method === 'POST') {
            // 准入模式：strict（默认，只认已登记）/ auto（首次发言即登记）/ open（不检查）
            const body = await readBody(req)
            const mode = String((body && body.mode) || '').toLowerCase()
            if (!['strict', 'auto', 'open'].includes(mode)) {
              return send(res, { ok: false, error: 'mode must be strict | auto | open' }, 400)
            }
            try {
              const f = configPath()
              const keep = readJsonFile(f)
              ...
              mkdirSync(dirname(f), { recursive: true })
              writeFileSync(f, JSON.stringify(keep, null, 2))
              pushAction({ action: 'allowlist', ok: true, detail: mode })
              let restarted = 0
              if (body && body.restart && keeper) { try { restarted = keeper.startBridge('allowlist-change') || 0 } catch (e) { ctx.logger?.warn?.('wxbridge: 重启失败 ' + String(e?.message ?? e)) } }
              return send(res, { ok: true, allowlist: mode, configFile: f, restarted,
                note: restarted ? '已写入配置并重启桥，立即生效' : '已写入配置；重启桥后生效（面板「重启」或等下次宿主启动）' })
            } catch (e) { return send(res, { ok: false, error: String(e?.message ?? e) }, 500) }
          }
          if (p === '/prompt') {
            if (req.method === 'POST') {
