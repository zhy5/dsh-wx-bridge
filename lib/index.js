...
export function apply(ctx, config) {
  ...
  const boot = async () => {
    ...
    keeper = createKeeper({
      ...
      bridgeArgs: [...(vault ? ['--vault', vault] : []), ...(ownership === 'host' ? ['--owner-pid', String(process.pid), '--host-home', hostHome] : [])],
  ...
  ctx.inject?.(['webServer'], (webCtx) => {
    ...
    const disposer = webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          await booted
          if (p === '/status') { const b = bridgeView(); return send(res, { ok: true, plugin: 'wxbridge', version: '0.0.8', hostPid: process.pid, bridge: b, peers: b ? b.peers : 0 }) }
