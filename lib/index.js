...
export function apply(ctx, config) {
  ...
  const boot = async () => {
    ...
    try {
      ...
      writeFileSync(addrFile, JSON.stringify({
        pid: process.pid,
        at: new Date().toISOString(),
        hint: 'port 由宿主 --port 决定；本文件仅记录插件可用性，实际端口见宿主启动日志',
      }, null, 2))
    } catch {}
  ...
  ctx.inject?.(['webServer', 'workspaceRegistry', 'sessions', 'agents', 'sessionPersistence'], (webCtx) => {
    ...
    const disposer = webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          await booted
          if (p === '/status') { const b = bridgeView(); return send(res, { ok: true, plugin: 'wxbridge', version: '0.0.14', hostPid: process.pid, bridge: b, peers: b ? b.peers : 0 }) }
          ...
          if (p === '/svc') {
            ...
            return send(res, { ok: true, services: seen, workspaceRegistryFace: reg, sessionsFace: ses, agentsFace: ag })
          }
