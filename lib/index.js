  ...
  ctx.inject?.(['webServer'], (webCtx) => {
    ...
    const disposer = webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          await booted
          if (p === '/status') { const b = bridgeView(); return send(res, { ok: true, plugin: 'wxbridge', version: '0.0.7', hostPid: process.pid, bridge: b, peers: b ? b.peers : 0 }) }
