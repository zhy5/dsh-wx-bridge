...
export function createKeeper(opts = {}) {
  ...
  /** 显式启动：清除人工停止意图，再拉起。 */
  function startBridgeNow(reason) {
  ...
  return { run, tick, status, startBridge: startBridgeNow, stopBridge, log, dataDir: DATA, cwd: CWD, bridgePath: BRIDGE, intervalMs: INTERVAL_MS, staleMs: STALE_MS, manualStop: readManualStop }
}
