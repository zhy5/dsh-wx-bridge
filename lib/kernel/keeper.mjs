 ...
 */
import { readFileSync, writeFileSync, appendFileSync, existsSync, mkdirSync, openSync, closeSync } from 'node:fs'
...
export function createKeeper(opts = {}) {
  ...
  function spawnBridge(detached = true) {
    ...
    let fd = 'ignore'
    const logFile = join(DATA, 'bridge-standalone.log')
    // 每次启动写一条分隔线：时间 + kernel 版本 + 关键参数。
    // 没有它，追加式日志会把"上一次的崩溃"混进"这一次的输出"，用户会误读（2026-09-21 实况）。
    try {
      mkdirSync(DATA, { recursive: true })
      let ver = '?'
      try { ver = JSON.parse(readFileSync(join(HERE, '..', '..', 'package.json'), 'utf8')).version || '?' } catch {}
      const sep = '===== bridge start ' + new Date().toISOString() + ' pid_parent=' + process.pid +
        ' ver=' + ver + ' data=' + DATA + ' cwd=' + CWD + ' detached=' + detached + ' ====='
      appendFileSync(logFile, '\n' + sep + '\n')
      fd = openSync(logFile, 'a')
    } catch (e) {
      // 出声：这里失败会让"桥为什么起不来"重新变成黑洞，绝不能静默（与 L3 E-2026-09-21-16/20 同族）
      console.error('[keeper] 写 bridge-standalone.log 失败：' + String(e && e.message ? e.message : e))
    }
    const child = spawn(NODE, [BRIDGE, ...extra], { cwd: CWD, env: childEnv(), detached, stdio: ['ignore', fd, fd], windowsHide: true })
