...
function loadRefs(excludeWechat) {
...
function loadWechatToken() {
  if (process.env.DSH_WEIXIN_BOT_TOKEN) return process.env.DSH_WEIXIN_BOT_TOKEN
  const f = join(DSH_HOME, '.credentials.yaml')
  const m = readFileSync(f, 'utf8').match(/^\s*DSH_WEIXIN_BOT_TOKEN:\s*(.+)$/m)
  if (!m) throw new Error('DSH_WEIXIN_BOT_TOKEN not found in ' + f)
...
function saveState(s) {
  ...
  stateChain = stateChain.then(() => { try { writeFileSync(tmp, JSON.stringify(s, null, 1)); renameSync(tmp, STATE_FILE) } catch (e) { log('state-write-failed', { error: String(e?.message ?? e) }) } })
  return stateChain
}
...
function runDsh(task, cwd, taskId) {
  return new Promise((resolve) => {
    ...
    const env = { ...process.env, DSH_HOME: CHILD_HOME }
...
let state = loadState()
const wechatToken = loadWechatToken()
...
async function handleMessage(token, msg) {
  ...
  if (!isAllowed && !authed) { log('rejected-unauthenticated', { from: from.slice(0, 12) }); return }
  if (!isAllowed && authed) {
...
async function main() {
  ...
  console.log('[bridge] 契约=' + CONTRACT_HASH + '｜子进程HOME=' + CHILD_HOME + '(workspace-write)')
