...
import { AcpHost, isAcpTimeout } from './acp.mjs'
...
// 一轮 ACP 提示词的等待上限（与"任务超时"分开）：
//   任务超时（TASK_TIMEOUT_MS，5 分钟）用在**兜底路径**上——那里超时是真杀子进程；
//   而 ACP 超时只意味着"我们不再等"，那一轮**还在跑**，所以给足 15 分钟，且超时不回落（见 runViaAcp）。
const PROMPT_TIMEOUT_MS = Number(process.env.BRIDGE_PROMPT_TIMEOUT_MS || ACP_CFG.promptTimeoutMs || 900000)
...
function getAcpHost(key) {
  ...
  if (!acpHosts.has(k)) acpHosts.set(k, new AcpHost({
    ...
    promptTimeoutMs: PROMPT_TIMEOUT_MS, log,
  }))
 ...
 * 返回 null 才意味着"ACP 不可用"，由调用方回落宿主/headless 路径；
 * **超时不算不可用**（那一轮还在跑）→ 返回 `{pending:true}` 并等 onLate 补发结果。
 */
async function runViaAcp(text, peer, taskId, peerKey) {
  if (!ACP_ENABLED) return null
  let sessionId = ''
  try {
    sessionId = await ensurePeerSession(peer)
    ...
    const r = await getAcpHost(peerMode(peer)).ask(sessionId, text, (u) => {
      ...
      onLate: (late) => deliverLateAcpResult(peer, peerKey, taskId, late),
    })
    ...
    if (isAcpTimeout(e)) {
      // **超时 ≠ 失败**：这一轮还在 ACP 侧跑，服务端最后一定会回帧（onLate 负责补发）。
      // 这里绝不能返回 null —— 那会让 processTask 去走兜底路径，把同一件事**再干一遍**（而且是拼接提示词的旧形态）。
      log('task-acp-still-running', { id: taskId, session: sessionId, waitedMs: PROMPT_TIMEOUT_MS, error: String(e?.message ?? e) })
      return { ok: false, pending: true, sessionId, error: '这一轮超过 ' + Math.round(PROMPT_TIMEOUT_MS / 60000) + ' 分钟仍在跑' }
    }
    // 真·通道不可用：回落之前先取消这一轮，避免"兜底路径与 ACP 同时跑"重复执行
    if (sessionId) {
      try { await getAcpHost(peerMode(peer)).cancel(sessionId); log('acp-fallback-cancel', { id: taskId, session: sessionId }) } catch {}
    }
...
/** 迟到的那一轮跑完了：把结果补发给用户（手机侧不必重发原话）。 */
function deliverLateAcpResult(peer, peerKey, taskId, late) {
  const t = state.tasks[taskId] || {}
  const audited = auditOutput('⏳（这一轮跑完了）' + NL + (late.error ? '执行失败：' + late.error.message : late.text))
  if (audited.hits.length) log('output-audit', { id: taskId, hits: audited.hits })
  t.status = late.error ? 'failed' : 'done'
  t.endedAt = new Date().toISOString()
  t.lateDelivered = true
  peer.history = ((peer.history || []).concat([{ u: t.u || '', a: late.text || String(late.error?.message || '失败'), ok: !late.error, at: t.endedAt, late: true }])).slice(-50)
  log('task-late-done', { id: taskId, ok: !late.error, outLen: (late.text || '').length, tools: late.toolCalls })
  saveState(state).catch(() => {})
  const key = peerKey || Object.keys(state.peers).find((k) => state.peers[k] === peer) || ''
  if (!key) return
  reply(wechatToken, key, audited.text, peer.lastContextToken).catch((e) => log('late-reply-failed', { error: String(e?.message ?? e) }))
}
...
async function processTask(token, peerKey, from, taskId, text, contextToken) {
  ...
  const t = state.tasks[taskId]
  t.status = 'running'; t.u = text; t.startedAt = new Date().toISOString(); activeTasks = runningCount()
  await saveState(state)
  ...
  r = await runViaAcp(text, peer, taskId, peerKey)
  // ①-b 超时 ≠ 失败：这一轮还在 ACP 侧跑，结果由 onLate 补发；**不走兜底**（否则同一件事被重跑一遍）
  if (r && r.pending) {
    t.pendingSince = new Date().toISOString(); activeTasks = runningCount()
    await saveState(state)
    return reply(token, from, '⏳ 这一轮已跑 ' + Math.round(PROMPT_TIMEOUT_MS / 60000) + ' 分钟仍在继续（会话 '
      + String(r.sessionId || '').slice(0, 12) + '…）。' + NL
      + '我**不会重复执行**它；跑完直接把结果发到这里（/task 看进度，/cancel 可中止）。', contextToken)
  }
  let viaFallback = ''
  // ② 其次宿主内执行（历史行为；不产生 turn 时返回空 → 继续回落）
  if (!r) { const viaHost = await runViaHost(text, peer, taskId); if (viaHost && String(viaHost.text || '').trim()) { r = viaHost; viaFallback = 'host' } }
  // ③ 最后回落：一次性 headless（沿用拼接提示词，仅作兜底）
  if (!r) { r = await runDsh(prompt, peer.cwd, taskId); viaFallback = 'headless' }
  ...
  log('task-done', { id: taskId, ok: r.ok, timeout: !!r.timedOut, viaFallback: viaFallback || 'acp', outLen: r.text?.length ?? 0 })
  const fallbackNotice = viaFallback
    ? '⚠️ 本轮 ACP 路径不可用，走了兜底（' + viaFallback + '）；兜底路径会把历史拼接进提示词（不是上下文回退）。' + NL
    : ''
  const audited = auditOutput((r.notice ? r.notice + NL : '') + fallbackNotice + (r.ok ? r.text : '执行失败：' + (r.error ?? '未知错误')))
  if (audited.hits.length) log('output-audit', { id: taskId, hits: audited.hits })
...
async function main() {
  ...
  if (args.includes('--print-config')) {
    // 只读自检：把桥实际认到的配置链打出来（不启动主循环、不抢锁、不发网络）。
    // 用途：核对"我改的那份 config.json / 预设"是不是桥正在用的那份（多 home 环境下最容易搞错）。
    console.log('[print-config] ' + JSON.stringify({
      dataDir: STATE_DIR, cwd: CWD,
      hostHome: HOST_HOME || '(未指定)', acpHome: ACP_HOME, childHome: CHILD_HOME,
      vault: VAULT_PATH || '(未指定)', contract: CONTRACT_FILE,
      acp: { enabled: ACP_ENABLED, preset: ACP_CFG.preset || '(跟随宿主默认)', permPolicy: ACP_PERM,
             permPreset: ACP_PERM_PRESET, approvalTimeoutMs: APPROVAL_TIMEOUT_MS, promptTimeoutMs: PROMPT_TIMEOUT_MS,
             taskTimeoutMs: TASK_TIMEOUT_MS, patchAcp: ACP_CFG.patchAcp !== false },
      overlays: { presetOverlay: overlayFor(ACP_DEFAULT_PRESET), chatOverlay: ensureChatOverlay() },
      push: { cli: PUSH_CLI, outbox: OUTBOX_DIR },
    }, null, 1))
    process.exit(0)
  }

  if (args.includes('--selftest-acp-timeout')) {
    // 离线自测：**超时 ≠ 失败** 这条规则的三段行为（不连模型、不起子进程）
    const rows = []
    const mk = (timeoutMs) => {
      const h = new AcpHost({ promptTimeoutMs: timeoutMs, permPolicy: 'ask', log: () => {} })
      h.ready = Promise.resolve({})                       // 跳过真实握手
      h.child = { stdin: { write: () => {} }, exitCode: null }
      return h
    }
    const frame = (id, text, stop) => JSON.stringify({ jsonrpc: '2.0', id, result: { stopReason: stop, _text: text } }) + NL
    // ① 分类：超时 vs 其它
    const timeoutErr = Object.assign(new Error('ACP 超时：session/prompt'), { code: 'ACP_TIMEOUT' })
    rows.push({ name: 'isAcpTimeout(tagged)', ok: isAcpTimeout(timeoutErr) === true })
    rows.push({ name: 'isAcpTimeout(plain message)', ok: isAcpTimeout(new Error('ACP 超时：session/prompt')) === true })
    rows.push({ name: 'isAcpTimeout(其它错误)', ok: isAcpTimeout(new Error('ACP 进程已退出(code=1)')) === false })
    // ② keepAlive：超时后迟到帧交给 onLate（正文靠 session/update 累积）
    {
      const h = mk(120)
      let late = null
      const id = h.nextId
      let code = ''
      try { await h.ask('s-late', 'hi', null, { timeoutMs: 120, keepAliveOnTimeout: true, onLate: (r) => { late = r } }) } catch (e) { code = e.code || '' }
      const keptAfterTimeout = h.pending.has(id)
      h._onData(Buffer.from(JSON.stringify({ jsonrpc: '2.0', method: 'session/update', params: { sessionId: 's-late', update: { sessionUpdate: 'agent_message_chunk', content: { type: 'text', text: '迟到正文' } } } }) + NL))
      h._onData(Buffer.from(frame(id, '', 'end_turn')))
      rows.push({ name: '超时错误带 code=ACP_TIMEOUT', ok: code === 'ACP_TIMEOUT' })
      rows.push({ name: '超时后 pending 条目被保留', ok: keptAfterTimeout === true })
      rows.push({ name: '迟到帧触发 onLate 且正文完整', ok: !!late && late.text === '迟到正文' && late.stopReason === 'end_turn' })
      rows.push({ name: '迟到后 pending 清空 + sink 摘除', ok: !h.pending.has(id) && !h.inflight.has('s-late') })
    }
    // ③ 对照：不 keepAlive 时超时即丢弃，迟到帧不会触发任何回调（旧行为）
    {
      const h = mk(120)
      let late = null
      const id = h.nextId
      let code = ''
      try { await h.ask('s-drop', 'hi', null, { timeoutMs: 120, onLate: (r) => { late = r } }) } catch (e) { code = e.code || '' }
      h._onData(Buffer.from(frame(id, '', 'end_turn')))
      rows.push({ name: '未开 keepAlive：超时即丢弃且无回调', ok: code === 'ACP_TIMEOUT' && late === null && !h.pending.has(id) && !h.inflight.has('s-drop') })
    }
    console.log('[selftest-acp-timeout] ' + JSON.stringify({
      rows, failed: rows.filter((r) => !r.ok).map((r) => r.name),
      promptTimeoutMs: PROMPT_TIMEOUT_MS, taskTimeoutMs: TASK_TIMEOUT_MS,
    }, null, 1))
    process.exit(rows.every((r) => r.ok) ? 0 : 1)
  }
  acquireLock()
