...
// 手机通道的权限预设（ACP 叠层里 permission.defaultPreset）。默认仍是最宽的 danger-full-access：
// 收紧到 workspace-write 会让 ~/.m2 之类的写操作被拦（maven 编译直接失败），所以交用户显式选择。
// 但**审批现在是可应答的**：一旦收紧，审批卡会推到微信，回「批准 / 拒绝」即可。
const ACP_PERM_PRESET = String(ACP_CFG.permPreset || 'danger-full-access')
...
function ensureChatOverlay() {
  ...
  if (chatOverlayPath) return chatOverlayPath
  let text = ''
  try { text = readFileSync(src, 'utf8') } catch { chatOverlayPath = src; return src }
  if (text.includes('wxpush.mjs')) { chatOverlayPath = src; return src }
  const lines = text.split(NL)
  while (lines.length && !String(lines[lines.length - 1]).trim()) lines.pop()
  const indent = (String(lines[lines.length - 1] || '').match(/^\s*/) || [''])[0]
  const body = lines.map((l) => (/^(\s*)defaultPreset:\s*danger-full-access\s*$/.test(l) ? l.replace('danger-full-access', ACP_PERM_PRESET) : l))
  const out = body.concat(pushHintLines().map((h) => indent + h)).join(NL) + NL
  try {
    const f = join(STATE_DIR, 'acp-overlay-chat.gen.yml')
    writeFileSync(f, out)
    chatOverlayPath = f
    log('chat-overlay-written', { file: f })
    return f
  } catch (e) { log('chat-overlay-failed', { error: String(e?.message ?? e) }); chatOverlayPath = src; return src }
}
