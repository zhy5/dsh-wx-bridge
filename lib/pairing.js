...
export function seedState(stateFile, { userId, accountId, baseUrl, addOwner, dropOthers, cwd }) {
  let s = {}
  try { s = JSON.parse(readFileSync(stateFile, 'utf8')) || {} } catch { s = {} }
  s.peers ||= {}; s.allowedUsers ||= []; s.tasks ||= {}; s.processed ||= []
  if (dropOthers && userId) {
    // 换了绑定账号：清掉旧主人，避免旧账号仍能驱动这台机器
    s.allowedUsers = []
    s.peers = {}
  }
  if (userId && (addOwner || s.allowedUsers.length === 0) && !s.allowedUsers.includes(userId)) s.allowedUsers.push(userId)
  if (userId) s.peers[userId] ||= { cwd: cwd || '', history: [], approvals: [] }
  ...
  return { allowedUsers: s.allowedUsers.length, account: s.account, registered: userId ? s.allowedUsers.includes(userId) : false }
}
...
export function createPairing(opts) {
  ...
  const finish = (resp) => {
    ...
    // 扫码的人就是新主人：直接登记进白名单（否则换号后消息会被"未登记"丢弃，
    // 还得再手工发一次登记口令 —— 这一步是 2026-09-21 换号踩坑后补的）。
    const seeded = seedState(stateFile, { userId, accountId, baseUrl, addOwner: true, dropOthers: true })
    session.status = 'confirmed'
    session.confirmed = {
      accountId, userId, baseUrl, credentialFile: credFile,
      allowedUsers: seeded.allowedUsers, registered: seeded.registered,
    }
    log('pair-confirmed account=' + accountId + ' user=' + String(userId).slice(0, 12)
        + ' cred=' + credFile + ' registered=' + seeded.registered)
  }
